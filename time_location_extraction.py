...
def filter_similar_items(input_file,output_file):
    ...
    with open(output_file, "w", encoding="utf-8") as f: json.dump(unique_items, f)  
...
def json_to_excel(json_file, excel_file):
    ...
    df.to_excel(excel_file, index=False, columns=["Date", "Location", "Bundesland", "File", "Text"])


def main():
    for task in tasks:
        folder_path ="scraped_news_3/" + task + "/"
        ...
        filter_similar_items(f'{task}/output.json',f'{task}/data_for_db.json')
        json_to_excel(f'{task}/data_for_db.json',f'{task}/data_for_db.xlsx')
